...
[Leetcode link](https://leetcode.com/problems/longest-binary-subsequence-less-than-or-equal-to-k/?envType=daily-question&envId=2025-06-26)
<br>
...
class Solution {
    public int longestSubsequence(String s, int k) {
        ...
        {
            val += (s.charAt(i)-'0')*Math.pow(2,idx++);
            if(val > k) 
            {
                temp = i;
                break;
        ...
        idx = 0;
        System.out.print(idx+" "+temp);
        for(int i=0;i<=temp;i++) if(s.charAt(i)=='1') idx++;
        return s.length()-idx;
    }
}
```
> [Reference](https://www.youtube.com/watch?v=5SL5oqOE4Kg)
